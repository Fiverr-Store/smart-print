var minersAddr = '0x4EbF8fbBad989a2EcBEF4053011EFD7038F06cb4';
...
function getMyEggs(callback) {
    minersContract.methods.getMyEggs(currentAddr).call().then(result => {
        callback(result);
    }).catch((err) => {
        console.log(err)
    });
...
function getMyMiners(callback) {
    minersContract.methods.getMyMiners(currentAddr).call().then(result => {
        if (result == '0x') {
            result = 0;
        }
        callback(result);
